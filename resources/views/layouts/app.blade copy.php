...
<body>
    ...
    <!-- 🎯 PHONE + OTP AUTHENTICATION JAVASCRIPT (FIXED) -->
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const sendMsg = document.getElementById('otpSendMsg');
        ...
        function startTimer() {
            ...
            if (timerCount) timerCount.innerText = seconds;
        ...
        if (sendOtpBtn) {
            sendOtpBtn.addEventListener('click', function() {
                ...
                if (!phone || phone.length !== 10 || !/^[6-9]\d{9}$/.test(phone)) {
                    sendMsg.innerHTML = '<div class="alert alert-danger py-1 small mb-2">Please enter a valid 10-digit Indian mobile number.</div>';
                    phoneInput.focus();
                    return;
                }
                ...
                .then(data => {
                    ...
                    sendOtpBtn.innerHTML = 'Get OTP <i class="bi bi-arrow-right ms-1"></i>';
                    if (data.status === 'success') {
                        ...
                        otpInput.value = '';
                        otpInput.focus();
                        verifyMsg.innerHTML = `<div class="alert alert-success py-1 small mb-2">${data.message}</div>`;
                        startTimer();
                    } else {
                        sendMsg.innerHTML = `<div class="alert alert-danger py-1 small mb-2">${data.message || 'Error sending OTP'}</div>`;
                    }
                })
                .catch(() => {
                    sendOtpBtn.disabled = false;
                    sendOtpBtn.innerHTML = 'Get OTP <i class="bi bi-arrow-right ms-1"></i>';
                    sendMsg.innerHTML = '<div class="alert alert-danger py-1 small mb-2">Network error. Please try again.</div>';
                });
            });
        }
        ...
        if (verifyOtpBtn) {
            verifyOtpBtn.addEventListener('click', function() {
                ...
                if (!otp || otp.length !== 4) {
                    verifyMsg.innerHTML = '<div class="alert alert-danger py-1 small mb-2">Please enter the 4-digit OTP.</div>';
                    otpInput.focus();
                    return;
                }
                ...
                fetch("{{ route('api.auth.verifyOtp') }}", {
                    ...
                    body: JSON.stringify({ 
                        otp: otp, 
                        phone: activePhone || phoneInput.value.trim(), 
                        name: name 
                    })
                })
                .then(res => res.json())
                .then(data => {
                    ...
                    verifyOtpBtn.innerHTML = 'Verify & Continue <i class="bi bi-check-circle ms-1"></i>';
                    if (data.status === 'success') {
                        verifyMsg.innerHTML = `<div class="alert alert-success py-1 small mb-2">${data.message}</div>`;
                        setTimeout(() => {
                        ...
                        verifyMsg.innerHTML = `<div class="alert alert-danger py-1 small mb-2">${data.message || 'Invalid OTP'}</div>`;
                    }
                })
                .catch(() => {
                    verifyOtpBtn.disabled = false;
                    verifyOtpBtn.innerHTML = 'Verify & Continue <i class="bi bi-check-circle ms-1"></i>';
                    verifyMsg.innerHTML = '<div class="alert alert-danger py-1 small mb-2">Verification failed. Try again.</div>';
                });
            });
        }
    ...
    <!-- Global Enquiry Modal Form Submit Script -->
    <script>
