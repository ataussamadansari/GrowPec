        ...
        <div class="leads-toolbar">
            <div class="filter-grid">
                ...
                <select id="sourceFilter" class="form-select crm-filter">
                    ...
                    @foreach($allLeads->pluck('source')->filter()->unique()->sort()->values() as $source)
                        <option value="{{ strtolower($source) }}">{{ ucwords(str_replace('_',' ', $source)) }}</option>
                    @endforeach
                </select>
                ...
                <a href="{{ route('admin.leads.export') }}?search={{ urlencode(request('search','')) }}&status={{ urlencode(request('status','')) }}" class="btn export-btn">
        ...
        <div class="table-shell">
            <table class="table leads-table align-middle mb-0" id="leadsTable">
                ...
                @forelse($leads as $lead)
                    ...
                    <tr
                        class="lead-row"
                        data-search="{{ strtolower(trim(($lead->name ?? '').' '.($lead->phone ?? '').' '.($lead->email ?? '').' '.($lead->city ?? '').' '.($lead->state ?? '').' '.($collegeName ?? '').' '.($courseName ?? '').' '.($specializationName ?? '').' '.($lead->source ?? ''))) }}"
                        data-status="{{ strtolower($lead->status ?? '') }}"
                        data-mode="{{ strtolower($mode ?? '') }}"
                    ...
                    >
                        ...
                        <td>
                            <div class="student-cell">
                                <span class="student-avatar">
                                    {{ strtoupper(substr(trim($lead->name ?: 'S'), 0, 1)) }}
                                </span>
                                <div class="student-info">
                                    <strong title="{{ $lead->name }}">{{ $lead->name }}</strong>
                                    <small title="{{ $lead->email ?? '' }}">
                        ...
                        <td>
                            <div class="contact-cell">
                                ...
                                @endif
                                @if($waPhone)
                                    <a href="https://wa.me/{{ $waPhone }}" target="_blank" rel="noopener noreferrer" class="whatsapp-btn">
                        ...
                        <td>
                            <div class="location-cell">
                                <div class="location-main">
                                    <i class="bi bi-geo-alt-fill me-1"></i>{{ $lead->city ?: 'N/A' }}
                                </div>
                                <div class="location-sub">
                                    {{ $lead->state ?: (optional($lead->stateRelation)->name ?? 'State not set') }}
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            @if($collegeName)
                                <div class="college-cell" title="{{ $collegeName }}">
                                    <div class="college-main">{{ $collegeName }}</div>
                                    @if(optional($lead->college)->college_type)
                                        <div class="location-sub">{{ optional($lead->college)->college_type }} University</div>
                                    @endif
                                </div>
                            @else
                        ...
                        <td>
                            <div class="program-cell">
                                <div class="program-main">{{ $courseName ?: 'Not selected' }}</div>
                                @if(optional($lead->course)->level)
                                    <div class="program-sub">{{ optional($lead->course)->level }}</div>
                                @endif
                            </div>
                        </td>
                        ...
                        <td>
                            @if($specializationName)
                                <span class="type-badge">
                                    ...
                                    {{ is_numeric($specializationName) ? 'ID #'.$specializationName : $specializationName }}
                                </span>
                            @else
                        ...
                        <td>
                            @if($mode)
                                <span class="mode-badge {{ strtolower($mode) === 'online' ? 'mode-online' : 'mode-regular' }}">
                                    <i class="bi {{ strtolower($mode) === 'online' ? 'bi-laptop' : 'bi-building' }}"></i>
                                    {{ ucfirst($mode) }}
                                </span>
                            @else
                        ...
                        <td>
                            <div class="source-cell">
                                <span class="source-chip" title="{{ $lead->source }}">
                                    <i class="bi bi-bullseye me-1"></i>
                                    {{ ucwords(str_replace('_',' ', $lead->source ?: 'Unknown')) }}
                                </span>
                                @if($lead->utm_source)
                                    <span class="source-sub">UTM: {{ $lead->utm_source }}</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td>
                            @if($followup)
                                <div class="followup-cell">
                                    <span class="followup-date {{ $followupClass }}">
                                        <i class="bi bi-calendar-event followup-icon me-1"></i>
                                        {{ $followup->format('d M Y') }}
                                    </span>
                                    <span class="location-sub">
                                        {{ $followup->format('h:i A') }}
                                        @if($followupClass === 'overdue') · Overdue @elseif($followupClass === 'today') · Today @endif
                                    </span>
                                </div>
                            @else
                        ...
                        <td>
                            @if($assignedName)
                                <div class="assigned-cell">
                                    <span class="assigned-avatar">{{ strtoupper(substr($assignedName,0,1)) }}</span>
                                    <span class="assigned-name" title="{{ $assignedName }}">{{ $assignedName }}</span>
                                </div>
                            @else
                        ...
                        <td>
                            <form action="{{ route('admin.leads.updateStatus', $lead->id) }}" method="POST" class="status-form">
                                @csrf
                        ...
                        <td>
                            <div class="action-wrap">
                                <button
                                    ...
                                    data-city="{{ e($lead->city ?? '') }}"
                                    data-state="{{ e($lead->state ?? '') }}"
                                    data-college="{{ e($collegeName ?? 'General Inquiry') }}"
                                    data-course="{{ e($courseName ?? 'Not selected') }}"
                                    data-specialization="{{ e(is_numeric($specializationName ?? '') ? 'ID #'.$specializationName : ($specializationName ?? 'General / Core')) }}"
                                    data-mode="{{ e($mode ?? 'Not specified') }}"
                                    data-source="{{ e($lead->source ?? 'Unknown') }}"
                                    data-status="{{ e(ucfirst($lead->status ?? 'new')) }}"
                                    data-followup="{{ e($followup ? $followup->format('d M Y, h:i A') : 'Not scheduled') }}"
                                    data-assigned="{{ e($assignedName ?? 'Unassigned') }}"
                                    data-message="{{ e($message ?? 'No notes or message added.') }}"
                                >
                                    <i class="bi bi-eye"></i>
                                </button>
                                ...
                                <a href="tel:{{ $lead->phone }}" class="icon-action gold" title="Call Lead">
