...
<div class="course-{{ $isEdit ? 'edit' : 'create' }}-page">
    <div class="course-{{ $isEdit ? 'edit' : 'create' }}-wrap">
        <div class="{{ $isEdit ? 'edit' : 'create' }}-hero">
            <div class="hero-content">
                ...
                <div>
                    ...
                    <p>{{ $isEdit ? 'Update this academic program in the GrowPec course catalog.' : 'Create a new academic program for the GrowPec course catalog.' }}</p>
                </div>
            </div>
            @if($isEdit)
            <span class="course-name-pill"><i class="bi bi-book-half"></i>{{ $course->name }}</span>
            @endif
        </div>
        <div class="{{ $isEdit ? 'edit-card' : 'create-card' }}">
            <div class="form-heading"><span class="form-heading-icon"><i class="bi bi-mortarboard-fill"></i></span>Course Details</div>
            @if ($errors->any())
            <div class="error-box">
                <div class="fw-bold"><i class="bi bi-exclamation-triangle-fill me-1"></i>Please fix the following:</div>
                <ul>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul>
            </div>
            @endif
            <form class="course-common-form" action="{{ $isEdit ? route('admin.courses.update', $course->id) : route('admin.courses.store') }}" method="POST">
                @csrf
                @if($isEdit)
                @method('PUT')
                @endif
                            ...
                            <div class="field-group mb-0">
                                ...
                                <div class="select-shell">
                                    ...
                                    <select id="courseLevel" name="level" class="form-select course-select" required>
                                        <option value="UG" {{ old('level', $course->level ?? 'UG') == 'UG' ? 'selected' : '' }}>
                                            Undergraduate (UG)
                                        </option>
                                        <option value="PG" {{ old('level', $course->level ?? '') == 'PG' ? 'selected' : '' }}>
                                            Postgraduate (PG)
                                        </option>
                                        <option value="Diploma" {{ old('level', $course->level ?? '') == 'Diploma' ? 'selected' : '' }}>
                                            Diploma
                                        </option>
                                        <option value="PhD" {{ old('level', $course->level ?? '') == 'PhD' ? 'selected' : '' }}>
                                            Ph.D. / Doctorate
                                        </option>
                                        <option value="Certificate" {{ old('level', $course->level ?? '') == 'Certificate' ? 'selected' : '' }}>
                                            Certificate
                                        </option>
                                    </select>
                                </div>
                            ...
                            <div class="field-group mb-0">
                                ...
                                <div class="select-shell">
                                    ...
                                    <select id="degreeType" name="degree_type" class="form-select course-select" required>
                                        <option value="Degree" {{ old('degree_type', $course->degree_type ?? 'Degree') == 'Degree' ? 'selected' : '' }}>
                                            Degree
                                        </option>
                                        <option value="Diploma" {{ old('degree_type', $course->degree_type ?? '') == 'Diploma' ? 'selected' : '' }}>
                                            Diploma
                                        </option>
                                        <option value="Certificate" {{ old('degree_type', $course->degree_type ?? '') == 'Certificate' ? 'selected' : '' }}>
                                            Certificate
                                        </option>
                                    </select>
                                </div>
                ...
                <div class="field-group">
                    ...
                    <div class="input-shell">
                        ...
                        <input
                            ...
                            id="courseDuration"
                            name="duration"
                            value="{{ old('duration', $course->duration ?? '3 Years') }}"
                ...
                <div class="form-actions">
                    <button type="submit" class="btn action-btn {{ $isEdit ? 'update-btn' : 'save-btn' }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.querySelector('.course-common-form');
        form?.addEventListener('submit', function() {
            const button = this.querySelector('button[type="submit"]');
            if (button && this.checkValidity()) {
